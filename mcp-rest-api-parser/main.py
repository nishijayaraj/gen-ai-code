...
def _query_reports_impl() -> Dict[str, Any]:
    """Fetch full reports list from the REST API and return items for LLM searching.
    Does NOT perform server-side filtering, sorting or pagination."""
    url = BASE_URL
...
_QUERY_REPORTS_SCHEMA: Dict[str, Any] = {
    "type": "object",
    "properties": {
        "priority": {"type": "string", "description": "(IGNORED) kept for compatibility; server does not filter."},
        "title": {"type": "string", "description": "(IGNORED) kept for compatibility; server does not filter."},
        "description": {"type": "string", "description": "(IGNORED) kept for compatibility; server does not filter."},
    },
...
async def list_tools() -> List[types.Tool]:
    return [
        types.Tool(
            name="query_reports",
            description="Return full reports list (no server-side filtering/sorting/pagination). LLM will search priority/title/description.",
...
async def call_tool(name arguments) -> List[types.TextContent]:
    ...
    # ignore incoming filter/paging args; return full list for LLM
